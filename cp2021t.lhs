...
\end{code}
Comecemos então por analisar o diagrama e a definição correspondente ao |outExpAr|:

\begin{eqnarray*}
\xymatrix@@C=4cm{
|ExpAr A| \ar@@/^1pc/[r]^-{|outExpAr|} & |1 + (A + (BinOp >< (ExpAr a >< ExpAr a)) + (UnOp >< ExpAr a)))| \ar@@/^1pc/[l]^-{|inExpAr|}
}
\end{eqnarray*}

\begin{code}
outExpAr X = Left ()
outExpAr(N x) = Right $ Left x
outExpAr(Bin op a b) = Right $ Right $ Left (op,(a,b))
outExpAr(Un op a) = Right $ Right $ Right(op,a)
\end{code}
...
Dito isto, definimos o |recExpAr| da seguinte maneira:
...
\\
     |A|
&
     |1 + A + (BinOp >< expn ExpAr (2)) + (UnOp >< ExpAr)|
           \ar[l]^-{|g|}
}
\end{eqnarray*}
...
&
    |1 + A + (BinOp >< expn ExpAr (2)) + (UnOp >< ExpAr)|
           \ar[d]^{|recExpAr (cataNat g_eval_exp)|}
\\
     |A|
&
     |1 + A + (BinOp >< expn A (2)) + (UnOp >< A)|
           \ar[l]^-{|g|}
}
\end{eqnarray*}

O gene do catamorfimo |eval_exp|, |g_eval_exp|, corresponde ao cálculo das expressões de acordo com o data type.

\begin{code}
g_eval_exp x (Left _) = x
g_eval_exp x (Right  (Left a)) = a
g_eval_exp _ (Right (Right (Right (Negate,n)))) = -n 
...
O |clean| é o anamorfismo que "corta" os ramos absorventes das expressões. A sua definição apresenta-se a seguir:
...
O |gopt| faz a otimização restante.
\textbf{Nota}: Alguns casos dão NaN.  

\begin{code}
gopt x (Left ()) = x
gopt _ (Right(Left a)) = a 
gopt _ (Right(Right(Left (Product, (a, 1))))) = a
gopt _ (Right(Right(Left (Product, (1, a))))) = a
gopt _ (Right(Right(Right (Negate, 0)))) = 0
gopt _ (Right(Right(Left (Sum, (a, b))))) = a + b
gopt _ (Right(Right(Left (Product, (a, b))))) = a * b
gopt _ (Right(Right(Right (Negate, a)))) = negate a 
gopt _ (Right(Right(Right (E, a)))) = expd a

\end{code}

O |sd_gen| corresponde ao gene do catamorfimo da diferenciação simbólica, que avalia o valor
da expressão passo a passo, devolvendo um par, sendo a sua primeira componente a expressão original e a segunda a derivada.


\begin{eqnarray*}
\xymatrix@@C=4cm{
       |ExpAr A| \ar[d]_-{|(cataNat (sd_gen)|}
            \ar[r]^-{|outExpAr|}
&
    |1 + A + (BinOp >< expn ExpAr (2)) + (UnOp >< ExpAr)|
...
sd_gen(Right (Right (Left (Sum, ((f, g),(h, i))))))  = (Bin Sum f h, Bin Sum g i)
sd_gen(Right (Right (Left (Product, ((f, g),(h, i))))))  = (Bin Product f h, (Bin Sum (Bin Product f i) (Bin Product g h)))
sd_gen(Right (Right (Right (Negate, (f, g))))) = (Un Negate f, Un Negate g)
sd_gen(Right (Right (Right (E, (f,g))))) = (Un E f, Bin Product (Un E f) g )
sd_gen (Left ()) = (X, N 1) 
\end{code}

O |ad_gen| corresponde ao gene do catamorfimo da diferenciação simbólica, que avalia o valor
...
\begin{code}
ad_gen _ (Right (Left a)) = (N a, 0)
ad_gen _ (Right (Right (Left (Sum, ((f, g),(h, i))))))  = (Bin Sum f h, g + i)
ad_gen a (Right (Right (Left (Product, ((f, g),(h, i))))))  = (Bin Product f h, i * eval_exp a f  + g * eval_exp a h)
ad_gen _ (Right (Right (Right (Negate, (f, g))))) = (Un Negate f, negate g)
ad_gen a (Right (Right (Right (E, (f,g))))) = (Un E f, (expd (eval_exp a f))*g)
ad_gen _ (Left()) = (X, 1)
\end{code}
...
Definir
\begin{code}
loop (c, a, b, s, r) = (div (c * a) b, a+s, b+r, s+8, r+2)
inic = (1, 2, 2, 10, 4)
prj (c, a, b, s, r) = c
\end{code}
por forma a que
...
Agora iremos calcular |C_n+1| a partir da fórmula anterior
...
\end{eqnarray}
Separando a parte recursiva |C_n| da não recursiva 
\begin{eqnarray}
  \frac{(2n+2) (2n+1)}{(n+2) (n+1) } 
\end{eqnarray}
Esta última vai ser divida em duas funções |a|(|n|) e |b|(|n|), que designam, respetivamente, o numerador
...
Para obter |a|(|n|) e |b|(|n|) recursivamente consideremos respetivamente as funções |s|(|n|) e |r|(|n|)
...
Passemos agora para a análise da definição da função |calcLine|, começando por visualizar o respetivo diagrama.
...
De seguida apresentamos a implementação da função |deCasteljau|:

\begin{code}
deCasteljau :: [NPoint] -> OverTime NPoint
deCasteljau = hyloAlgForm alg coalg where
   coalg [] = Left()
   coalg [x] = Right $ Left x
   coalg l = Right $ Right (init l, tail l)
   alg = either (const nil) (either const calcLineLambda)
   calcLineLambda (a,b) = \pt -> calcLine (a pt) (b pt) pt

hyloAlgForm f g = f . (id -|- (id -|- (hyloAlgForm f g >< hyloAlgForm f g))) . g
...
Assim sendo, temos o seguinte diagrama que corresponde à definição da função |avg_aux|:
\begin{eqnarray*}
\xymatrix@@C=4cm{
       |LTree A| \ar[d]_-{|avg_aux|}
            \ar[r]^-{|outLTree|}
&
    |A + expn((LTree A)) (2)|
           \ar[d]^{|id + expn((avg_aux)) (2) |}
\\
     |expn((N0)) (2)|
&
     |A >< expn ((expn  (N0)(expn (2)))) (2)|
           \ar[l]^-{|g|}
}
\end{eqnarray*}

\begin{code}
avg_aux = cataNEList (either b q) 
   where
     b = split h1 k1
     q = split h2 k2
     h1 = id
...
\begin{eqnarray*}
\xymatrix@@C=4cm{
       |LTree A| \ar[d]_-{|avgLTree|}
            \ar[r]^-{|outLTree|}
&
    |A + expn((LTree A)) (2)|
           \ar[d]^{|id + expn((avgLTree)) (2) |}
\\
     |N0|
&
     |A >< expn ((expn  (N0)(expn (2)))) (2)|
